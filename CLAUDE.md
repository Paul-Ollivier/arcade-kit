...
Bun resolves the `#fragment` as a git ref, **not** an npm semver range — `#semver:^1.0.0` does NOT work. Current published tag: **v1.15.0** (tightened the basic/body font letter-spacing by 1px — 7px advance on the 8px cell — across all four renderers: DOM `BitmapText`, Pixi `FONT_BASIC`, canvas, and the web font; v1.14.0 added the pixel **web font** — `loadPixelWebFont`/`PIXEL_FONT_FAMILY` + `assets/d8-pixel.woff2` — so CSS-styled DOM text matches the BitmapText face; v1.13.0 formalized the **typography** system — `TYPE_SCALE`, `FONT_BODY`/`FONT_TITLE`, `makeBitmapText`, canvas `drawBitmapText`, recoloured the basic atlas white). Tags follow semver intent (breaking visual/API change → major). After rollout the hub + all games pin v1.14.0.
