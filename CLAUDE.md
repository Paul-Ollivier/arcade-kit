...
- `GOLD_CUP_URL` + `GOLD_CUP_SIZE` (`trophy.ts`) — the gold trophy cup (20×16 native), the leaderboard's icon: the hub cabinet's control-panel cup button, board headers, rank chips. Wider than it is tall, so size it by one axis and derive the other from `GOLD_CUP_SIZE` rather than forcing a square box — and read those numbers off the PNG itself when the art is re-cut. The art carries its own gold palette and dark outline — it reads on a light or dark cap with no tinting. Plain URL string; render `image-rendering: pixelated` at a whole-number scale.
