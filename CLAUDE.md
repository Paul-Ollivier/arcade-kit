...
`fonts/` (bitmap-font atlases + the woff2), `ui/` (buttons, panels, chat bubble, glove, nav arrows, dither gradients), `sprites/` (golden coin + vault, plus `jewels/` and `bars/` — the loot props), `audio/` (raw mp3 sources for `/sfx`), `game/` (fighter/VFX atlases, hp bars, props). First-party files are `d8-<kebab>.<ext>`; every asset is consumed via an exported URL constant, never deep-imported by consumers.
